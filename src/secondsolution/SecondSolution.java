        ...
        try {
            ...
            switch (number) {
                case 1:
                    ...
                    listCity.stream().filter(line -> country.equals(line.nameOfConutry)).forEach(new Consumer<City>() {
                        @Override
                ...
                case 2:
                    ...
                    Info.secondInfo(text);
                    listCity.stream().filter(line -> "Bulgaria".equals(line.nameOfConutry)).forEach(new Consumer<City>() {
                        @Override
                        public void accept(City s) {
                ...
                case 3:
                    ...
                    timeStart = startInMiliSeconds();
                    int count = 0;
                    count = (int) listCity.stream().filter(line -> line.name.startsWith(str)).count();
                    listCity.stream().filter(line -> line.name.startsWith(str)).forEach(new Consumer<City>() {
                        @Override
                        public void accept(City s) {
                            String str = (s.name + "\n\r");
                            try {
                ...
                case 4:
                    ...
                    Info.secondInfo(text);
                    listCity.stream().filter(line -> city.equals(line.name)).forEach(new Consumer<City>() {
                        @Override
                        public void accept(City s) {
